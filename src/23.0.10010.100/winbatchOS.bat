...
rem settings revision 4
	...
	IF %_SETTINGS.SECTION%==SYSTEM.DISPLAY IF %I%==m IF %M% EQU 1 IF %X% GTR 122 IF %Y% GTR 8 IF %X% LSS 127 IF %Y% LSS 10 CALL Text 65 8 f0 "                                                                  " X _Button_Boxes _Button_Hover
		...
		rem Yes
			IF %_SETTINGS.SECTION%==SYSTEM.RECOVERY.RESET IF %I%==m IF %M% EQU 1 IF %X% GTR 92 IF %Y% GTR 31 IF %X% LSS 108 IF %Y% LSS 35 goto :SETTINGS.RECOVERY.RESET
		rem No
			IF %_SETTINGS.SECTION%==SYSTEM.RECOVERY.RESET IF %I%==m IF %M% EQU 1 IF %X% GTR 130 IF %Y% GTR 31 IF %X% LSS 146 IF %Y% LSS 35 goto :SETTINGS.RECOVERY.RESET
...
rem :TERMINAL.LOGIN
rem echo User premission required.
rem echo If you want to exit, type 'exit' now.
rem echo User: %_WBX_USERNAME%
rem echo.
rem SET _PASS=0
rem SET /p _PASS=Password:
rem IF %_WBX_PASSWORD%==%_PASS% goto :TERMINAL.SYSTEMLOOP
rem IF %_PASS%==exit GOTO :TERMINAL.LOOP
rem IF %_PASS%==0 GOTO :TERMINAL.LOGIN
rem goto :TERMINAL.LOGIN
...
IF %WBXprompt%==cd goto :TERMINAL.CD
IF %WBXprompt%==cls goto :TERMINAL.CLS
IF %WBXprompt%==dir goto :TERMINAL.DIR
...
rem list of commands here
	...
	:TERMINAL.WBXINSTALL 
	echo.
	echo OSverificationError
	echo.
	goto :TERMINAL.SYSTEMLOOP

	For /F "Delims= Tokens=1,2,3,4" %%A in ('dir') Do (
	set dir-temp=%%A
	)
	echo %dir-temp%
